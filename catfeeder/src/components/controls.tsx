...
export default function Controls() {
  ...
  // Update the motor speed via the backend API.
  const updateMotorSpeed = async (value: number) => {
    setBrightness(value);
    try {
      await fetch(`/api/py/brightness/${value}`, { method: "PUT" });
    } catch (error) {
...
useEffect(() => {
  const fetchSounds = async () => {
    ...
    try {
      const response = await fetch("http://localhost:5001/api/get-sounds");
...
const playSound = async () => {
  ...
  try {
    const response = await fetch(
      `http://localhost:5001/api/play-sound/${encodeURIComponent(selectedSound)}`,
      { method: "POST" }
    );
